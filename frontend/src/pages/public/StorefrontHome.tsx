...
export default function StorefrontHome() {
  ...
  const { data: products = [], isLoading } = useQuery({
    ...
    queryFn: () => {
      ...
      return api.get(`/public/products?${params.toString()}`).then(r => r.data);
    }
  });
      ...
      <div className="bg-gradient-to-b from-[#1a1c23] to-[#0f1115] pt-20 pb-16 border-b border-white/5 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          ...
          >
            ...
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#ff9900] to-[#ffb84d]">Curated Products</span>
          </motion.h1>
          <motion.p 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            We manually review and verify the best deals and products across Amazon, Flipkart, and more so you don't have to.
          </motion.p>
          ...
          >
            ...
            />
            <button type="submit" className="absolute right-2 bg-[#ff9900] hover:bg-[#e68a00] text-black font-bold py-2.5 px-6 rounded-full transition-colors">
              Search
            </button>
          </motion.form>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-10">
            ...
            {CATEGORIES.map(cat => (
              <button 
                ...
                className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${
                  category === cat ? 'bg-white/15 text-white border-white/20' : 'bg-transparent text-white/50 hover:text-white hover:bg-white/5 border border-transparent hover:border-white/10'
                }`}
              >
                {cat}
              </button>
          ...
          <div className="flex items-center gap-2 bg-white/5 rounded-xl p-1 border border-white/5">
            ...
            >
              <option value="latest" className="bg-[#1a1c23]">Latest Added</option>
              <option value="price_asc" className="bg-[#1a1c23]">Price: Low to High</option>
              <option value="price_desc" className="bg-[#1a1c23]">Price: High to Low</option>
              <option value="rating" className="bg-[#1a1c23]">Top Rated</option>
            </select>
          </div>
        </div>
        ...
        {/* Product Grid */}
        {isLoading ? (
          <div className="flex justify-center py-20"><div className="w-10 h-10 border-4 border-[#ff9900] border-t-transparent rounded-full animate-spin" /></div>
        ) : products.length === 0 ? (
          <div className="text-center py-20 bg-white/5 rounded-2xl border border-white/5">
            <p className="text-xl text-white/40 mb-2">No products found.</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {products.map((p: any, i: number) => (
              ...
              >
                ...
                <a href={`/api/public/redirect/${p._id}`} target="_blank" rel="noreferrer" className="block relative aspect-square bg-white/5 p-4 overflow-hidden">
                  {p.imageUrl ? (
                    <img src={p.imageUrl} alt={p.productName} className="w-full h-full object-contain mix-blend-normal group-hover:scale-105 transition-transform duration-500" />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center text-white/20 font-medium">No Image</div>
                  )}
                ...
                <div className="p-5 flex-1 flex flex-col">
                  ...
                  <Link to={`/products/${p._id}`} className="block flex-1 group/title">
                  ...
                  <div className="mt-4 flex items-center justify-between">
                    ...
                    <Link to={`/products/${p._id}`} className="text-sm font-medium text-white/50 hover:text-white transition-colors">
                      Details
                    </Link>
                  </div>
                </div>
