...
export default function UploadPage() {
  ...
  const [status, setStatus] = useState<
    "idle" | "uploading" | "polling" | "done" | "error"
  >("idle");
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (data.status === "completed") {
          ...
          setResult(data);
          setUrls({
            manualProductUrl: data.manualProductUrl || "",
  ...
  const handleSendTelegram = async () => {
    ...
    try {
      ...
      setResult({
        ...result,
        telegramSent: true,
        manualProductUrl: urls.manualProductUrl,
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-white/50">
          Upload any product screenshot for AI extraction.
        </p>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          {!file ? (
            <div
              {...getRootProps()}
              className={`glass border-2 border-dashed rounded-2xl p-12 text-center cursor-pointer transition-all ${isDragActive ? "border-primary/60 bg-primary/5" : "border-white/10 hover:border-white/20"}`}
            >
              <input {...getInputProps()} />
            ...
            <div className="glass rounded-2xl overflow-hidden">
              <div className="relative">
                ...
                <button
                  onClick={clear}
                  className="absolute top-3 right-3 glass p-1.5 rounded-lg hover:bg-white/10"
                >
                  <X size={16} />
                </button>
              </div>
              ...
              >
                ...
                {PIPELINE_STEPS.map((s, i) => (
                  <div key={s} className="flex items-center gap-3 text-sm">
                    ...
                    <span
                      className={i <= step ? "text-white/80" : "text-white/30"}
                    >
                      {s}
                    </span>
                  </div>
        ...
        <div>
          {result ? (
            ...
            >
              {/* Confidence Score */}
              <div
                className={`glass border p-4 rounded-2xl flex items-center justify-between ${getConfidenceBg(result.confidenceScore)}`}
              >
                <div>
                  ...
                  val(key) ? (
                    ...
                    >
                      ...
                      <div className="text-right">
                        ...
                        <div
                          className={`text-xs ${getConfidenceColor(conf(key))}`}
                        >
                          {conf(key)}%
                        </div>
                      </div>
              ...
              {(result.productLinks?.amazon ||
                ...
                <div className="glass p-4 space-y-2">
                  ...
                  {result.productLinks.amazon && (
                    <a
                      href={result.productLinks.amazon}
                      target="_blank"
                  ...
                  {result.productLinks.flipkart && (
                    <a
                      href={result.productLinks.flipkart}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center gap-2 text-sm text-primary hover:underline"
                    >
              ...
              <div className="glass p-4 space-y-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    value={urls.manualProductUrl}
                    onChange={(e) =>
                      setUrls((p) => ({
                        ...p,
                        manualProductUrl: e.target.value,
                      }))
                    }
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    value={urls.affiliateUrl}
                    onChange={(e) =>
                      setUrls((p) => ({ ...p, affiliateUrl: e.target.value }))
                    }
                  />
                </div>
              </div>
              ...
              <div className="flex gap-3 flex-wrap">
                <a
                  href={`${API_BASE_URL}/extract/${result._id}/export/json`}
                  className="btn-secondary flex-1 text-center py-2.5 text-sm min-w-[120px]"
                >
                  Export JSON
                </a>
                <button
                  onClick={handleSendTelegram}
                  className={`flex-1 py-2.5 text-sm flex items-center justify-center gap-2 min-w-[160px] ${result.telegramSent ? "btn-secondary text-emerald-400" : "btn-primary"}`}
                >
                  <Bot size={16} />{" "}
                  {result.telegramSent
                    ? "Resend to Telegram"
